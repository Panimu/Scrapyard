...
describe('weapon slots', () => {
  ...
  it('still caps a real run well under the array, base and upgraded alike', () => {
    ...
    //
    // MAX_WEAPONS IS THE BASE NOW, not the ceiling - Reinforced Mounts buys the fourth slot and
    // then the fifth (see World.maxWeapons) - so what has to hold is that even a fully upgraded
    // save stays inside the array. Asserted against the catalog's own ceiling rather than a
    // literal, so a wider slot upgrade cannot quietly overrun WEAPON_SLOTS.
    expect(MAX_WEAPONS).toBe(3);
