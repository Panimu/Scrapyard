...
public static class Progress
{
    ...
    ///
    /// <c>bankedThisRun</c> is how much of THIS RUN's credit tally has already reached the save. The
    /// caller carries it across calls and resets it when a run starts - it is the one piece of state
    /// banking needs and the one thing the save cannot answer for itself.
    /// </remarks>
    public static Earned Bank(Settings save, World world, ILevel level, HeroUnlocks roster,
                              ref long bankedThisRun)
    {
        ...
        if (credits > bankedThisRun)
        {
            long added = credits - bankedThisRun;
            bankedThisRun = credits;
        ...
        // CAREER TOTALS ARE A MAX, NOT A SUM. The run's own counters only grow, so taking the
        // larger of the two folds the run in without double-counting the seconds already banked.
