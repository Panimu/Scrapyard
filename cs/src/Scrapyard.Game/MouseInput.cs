...
public sealed class MouseInput
{
    ...
    /// <summary>Turns the wheel's running total into notches. See <see cref="WheelCounter"/>.</summary>
    private WheelCounter _wheel;
    ...
    /// <summary>The left button is held right now.</summary>
    /// <remarks>
    /// THE LEVEL, WHERE <see cref="LeftClicked"/> IS THE EDGE. Everything that ACTS wants the edge -
    /// a button must fire once however long it is held. A DRAG wants the level: it begins on the
    /// edge and continues for as long as the button is down, wherever the pointer has got to.
    /// </remarks>
    ...
    public void Sample(int surfaceDivisor)
    {
        ...
        LeftClicked = LeftDown && _prevLeft == ButtonState.Released;
        _prevLeft = state.LeftButton;

        WheelNotches = _wheel.Read(state.ScrollWheelValue);
