 ...
 *
 * x10 HP at x0.1437 speed is a wall that walks. Both halves matter: ten times the hit points would
 ...
 * come back.
 *
 * THE SPEED HAS CLIMBED IN STEPS - 0.0605, 0.0726, 0.0871 (a fifth at a time), 0.1307 (half
 * again), and now 0.1437 (a tenth more). In absolute terms a Heavy walks at 6.8 to 14.8 u/s
 * depending on which archetype the cycle is spending (it was 6.1 to 13.5 the step before). The
 * complaint behind every step was the same one: the back of a siege ring took long enough to
 * close that a player could simply leave, deal with something else and come back to find it
 * barely moved. Every figure below that is measured in "seconds of its walking" moves with each
 * step, and they are updated rather than left to rot - a derived number in a comment is worth
 ...
 * Cannon shell throwing it as far as it walks in eight seconds turned the wall into something you
 * could sweep aside for free. Half was not enough - at 190 impulse on a 0.5-mass body that was
 * still 95 u/s, ten seconds of its walking per shell. A quarter still moves it visibly; it is
 * not the Scraplord's outright immunity. It just stops one shell being worth more than the whole
 * approach. The push itself is unchanged by the speed above - it comes from mass and knockback
 * resistance, neither of which moved - so only how much of the Heavy's OWN walking it buys back
 * gets shorter as the Heavy gets faster.
 *
...
export interface FlavourDef {
  ...
  /**
   * SECONDS OF WALKING AT A MARK INSTEAD OF AT THE PLAYER. 0 is "chase the player from the first
   * tick", which is everything except the Heavy.
   *
   * A body with a fixation snapshots the player's position ONCE, at its own spawn, and walks at
   * that fixed point until the timer runs out - then reverts to ordinary pursuit. The point does
   * not update, and that is the design: a siege ring is centred on where you were STANDING when it
   * was sprung, so all fifty Heavies collapse on that one spot. Stand still and it is a closing
   * noose; step out of it and it becomes a landmark - a slow grey knot forming on the yard where
   ...
   *
   * FORTY-FIVE SECONDS for the Heavy, and it has come down twice. It was ninety, which held the
   * whole ring parked on an empty mark long after it had finished arriving - the turn is the
   * interesting half of this behaviour and ninety seconds is a long time to wait for it. Then
   * fifty, chosen to just outlast the collapse: a siege body crossing from the 520 u ring at
   * roughly 10 u/s needs about fifty-two seconds to reach the centre, so the fixation ended as the
   * knot finished forming.
   *
   * FORTY-FIVE DELIBERATELY ENDS IT BEFORE THAT, and the paragraph above used to argue it must
   * not: "the fixation has to outlast the collapse or the ring reverts to a plain horde mid-walk
   * and the knot never forms at all". That was an argument for the KNOT, and the knot is not the
   * only thing worth having. Turning while the ring is still closing means the mass reads as
   * hunting several seconds sooner, from a formation the player can still see the shape of - which
   * is the part of this behaviour anyone actually watches. What is lost is a fully-closed circle
   * standing on an empty mark, and that was always a picture rather than a threat.
   *
   * A body that ARRIVES early stands on the mark until the timer ends - see the arrival deadzone
   * in the seek pass.
   */
